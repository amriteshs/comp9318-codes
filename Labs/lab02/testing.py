from lab02.submission import buc_rec_optimized
import os
import pandas as pd
import numpy as np
...
def main():
    ...
    for i in range(len(test_files)):
        ...
        if not type(output_data) is pd.DataFrame:
            print("Wrong output type: " + str(type(output_data)))
            continue
        ...
        correct = None
        if output_data.shape != answer.shape:
            correct = False
        else:
